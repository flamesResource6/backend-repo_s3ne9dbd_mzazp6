import os
from fastapi import FastAPI, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel
from typing import List, Optional
from bson import ObjectId
from datetime import datetime

from database import db, create_document, get_documents
from schemas import Event, Booking

app = FastAPI(title="Local Events Booking API", version="1.0.0")

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)


@app.get("/")
def read_root():
    return {"message": "Local Events Booking API is running"}


@app.get("/test")
def test_database():
    response = {
        "backend": "✅ Running",
        "database": "❌ Not Available",
        "database_url": None,
        "database_name": None,
        "connection_status": "Not Connected",
        "collections": []
    }

    try:
        if db is not None:
            response["database"] = "✅ Available"
            response["database_url"] = "✅ Configured"
            response["database_name"] = db.name if hasattr(db, 'name') else "✅ Connected"
            response["connection_status"] = "Connected"
            try:
                collections = db.list_collection_names()
                response["collections"] = collections[:10]
                response["database"] = "✅ Connected & Working"
            except Exception as e:
                response["database"] = f"⚠️  Connected but Error: {str(e)[:50]}"
        else:
            response["database"] = "⚠️  Available but not initialized"
    except Exception as e:
        response["database"] = f"❌ Error: {str(e)[:50]}"

    response["database_url"] = "✅ Set" if os.getenv("DATABASE_URL") else "❌ Not Set"
    response["database_name"] = "✅ Set" if os.getenv("DATABASE_NAME") else "❌ Not Set"
    return response


# --------- Event Endpoints ---------
@app.post("/api/events", response_model=dict)
def create_event(event: Event):
    if db is None:
        raise HTTPException(status_code=500, detail="Database not configured")
    inserted_id = create_document("event", event)
    return {"id": inserted_id}


@app.get("/api/events", response_model=List[dict])
def list_events(city: Optional[str] = None, category: Optional[str] = None):
    if db is None:
        raise HTTPException(status_code=500, detail="Database not configured")
    filt = {}
    if city:
        filt["city"] = {"$regex": city, "$options": "i"}
    if category:
        filt["category"] = {"$regex": category, "$options": "i"}
    docs = get_documents("event", filt, limit=50)
    # Convert ObjectId to string
    for d in docs:
        d["id"] = str(d.get("_id"))
        d.pop("_id", None)
    return docs


@app.get("/api/events/{event_id}", response_model=dict)
def get_event(event_id: str):
    if db is None:
        raise HTTPException(status_code=500, detail="Database not configured")
    try:
        doc = db["event"].find_one({"_id": ObjectId(event_id)})
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid event id")
    if not doc:
        raise HTTPException(status_code=404, detail="Event not found")
    doc["id"] = str(doc.get("_id"))
    doc.pop("_id", None)
    return doc


# --------- Booking Endpoints ---------
@app.post("/api/bookings", response_model=dict)
def create_booking(booking: Booking):
    if db is None:
        raise HTTPException(status_code=500, detail="Database not configured")

    # Verify event exists and capacity
    try:
        ev = db["event"].find_one({"_id": ObjectId(booking.event_id)})
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid event id")
    if not ev:
        raise HTTPException(status_code=404, detail="Event not found")

    # Calculate current booked seats
    agg = list(db["booking"].aggregate([
        {"$match": {"event_id": booking.event_id}},
        {"$group": {"_id": None, "qty": {"$sum": "$quantity"}}}
    ]))
    booked_qty = agg[0]["qty"] if agg else 0
    remaining = max(0, int(ev.get("capacity", 0)) - booked_qty)

    if booking.quantity > remaining:
        raise HTTPException(status_code=400, detail=f"Only {remaining} seats left")

    inserted_id = create_document("booking", booking)
    return {"id": inserted_id, "remaining": remaining - booking.quantity}


@app.get("/api/events/{event_id}/availability", response_model=dict)
def availability(event_id: str):
    if db is None:
        raise HTTPException(status_code=500, detail="Database not configured")
    try:
        ev = db["event"].find_one({"_id": ObjectId(event_id)})
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid event id")
    if not ev:
        raise HTTPException(status_code=404, detail="Event not found")

    agg = list(db["booking"].aggregate([
        {"$match": {"event_id": event_id}},
        {"$group": {"_id": None, "qty": {"$sum": "$quantity"}}}
    ]))
    booked_qty = agg[0]["qty"] if agg else 0
    remaining = max(0, int(ev.get("capacity", 0)) - booked_qty)

    return {
        "event_id": event_id,
        "capacity": int(ev.get("capacity", 0)),
        "booked": booked_qty,
        "remaining": remaining
    }


if __name__ == "__main__":
    import uvicorn
    port = int(os.getenv("PORT", 8000))
    uvicorn.run(app, host="0.0.0.0", port=port)
